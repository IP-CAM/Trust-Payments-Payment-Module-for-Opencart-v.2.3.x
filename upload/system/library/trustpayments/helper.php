...
class TrustPaymentsHelper {
	...
	private function __construct(Registry $registry){
		if ($registry instanceof Registry && $registry->has('session') && $registry->has('config') && $registry->has('db')) {
			...
			$this->loggers = array(
				...
				self::LOG_DEBUG => new Log('trustpayments_debug.log'),
				self::LOG_INFO => new Log('trustpayments_info.log')
			);
		}
		else {
			throw new \Exception("Could not instatiate TrustPaymentsHelper, invalid registry supplied.");
		}
	}
	...
	public function compareStoredCustomerSessionIdentifier(){
		$data = $this->registry->get('session')->data;
		if (isset($data['trustpayments_customer']) && !empty($data['trustpayments_customer'])) {
			$id = $data['trustpayments_customer'];
		}
		else {
			return false;
		}
	...
	public function getAddress($key, $order_info = array()){
		...
		if (isset($order_info[$key . '_address'])) {
			$address = \TrustPaymentsHelper::mergeArray($address, $order_info[$key . '_address']);
		}
		if (isset($order_info[$key . '_address_id'])) {
			$address = \TrustPaymentsHelper::mergeArray($address, $address_model->getAddress($$order_info[$key . '_address_id']));
		}
		...
		if (empty($address)) {
			...
			if (isset($session['guest'][$key]) && is_array($session['guest'][$key])) { // billing only
				$address = \TrustPaymentsHelper::mergeArray($address, $session['guest'][$key]);
			}
			if (isset($session[$key][$key . '_address'])) { // shipping only
				$address = \TrustPaymentsHelper::mergeArray($address, $session[$key][$key . '_address']);
			}
			if (isset($session[$key . '_address']) && is_array($session[$key . '_address'])) {
				$address = \TrustPaymentsHelper::mergeArray($address, $session[$key . '_address']);
			}
		}
		return $address;
	}
	...
	public function refreshWebhook(){
		...
		$saved = $this->registry->get('config')->get('trustpayments_notification_url');
		if ($generated == $saved) {
			return;
		}
		$space_id = $this->registry->get('config')->get('trustpayments_space_id');
		\TrustPayments\Service\Webhook::instance($this->registry)->uninstall($space_id, $saved);
		\TrustPayments\Service\Webhook::instance($this->registry)->install($space_id, $generated);
		...
		$query = "UPDATE `$config` SET `value`='$generated' WHERE `store_id`='$store_id' AND `key`='trustpayments_notification_url';";
		$db->query($query);
		$this->registry->get('config')->set('trustpayments_notification_url', $generated);
	}

	public function log($message, $level = self::LOG_DEBUG){
		if ($this->registry->get('config')->get('trustpayments_log_level') >= $level) {
	...
	public function getSpaceId($store_id){
		...
		$query = "SELECT value FROM $table WHERE `key`='trustpayments_space_id' AND `store_id`='$store_id'";
	...
	public function hasRunningJobs(\TrustPayments\Entity\TransactionInfo $transaction_info){
		return \TrustPayments\Entity\CompletionJob::countRunningForOrder($this->registry, $transaction_info->getOrderId()) +
				\TrustPayments\Entity\VoidJob::countRunningForOrder($this->registry, $transaction_info->getOrderId()) +
				\TrustPayments\Entity\RefundJob::countRunningForOrder($this->registry, $transaction_info->getOrderId()) > 0;
	}

	public function isCompletionPossible(\TrustPayments\Entity\TransactionInfo $transaction_info){
		return $transaction_info->getState() == \TrustPayments\Sdk\Model\TransactionState::AUTHORIZED &&
				(\TrustPayments\Entity\CompletionJob::countRunningForOrder($this->registry, $transaction_info->getOrderId()) == 0) &&
				(\TrustPayments\Entity\VoidJob::countRunningForOrder($this->registry, $transaction_info->getOrderId()) == 0);
	}

	public function isRefundPossible(\TrustPayments\Entity\TransactionInfo $transaction_info){
		...
		$refunded_amount = 0;
		foreach (\TrustPayments\Entity\RefundJob::loadByOrder($this->registry, $transaction_info->getOrderId()) as $refund_job) {
			switch ($refund_job->getState()) {
				case \TrustPayments\Entity\RefundJob::STATE_SUCCESS:
					$refunded_amount += $refund_job->getAmount();
					break;
				case \TrustPayments\Entity\RefundJob::STATE_SENT:
				case \TrustPayments\Entity\RefundJob::STATE_PENDING:
				case \TrustPayments\Entity\RefundJob::STATE_MANUAL_CHECK:
					return false;
			}
		}
	...
	public function getTranslation($key){
		if ($this->registry->get('language')->get($key) == $key) {
			$this->registry->get('language')->load('extension/payment/trustpayments');
		}
	...
	public function dbTransactionLock($space_id, $transaction_id){
		...
		$table = DB_PREFIX . 'trustpayments_transaction_info';
	...
	public function translate($strings, $language = null){
		...
		if ($language) {
			try {
				$language_provider = \TrustPayments\Provider\Language::instance($this->registry);
	...
	public function getSuccessUrl(){
		return TrustPaymentsVersionHelper::createUrl($this->getCatalogUrl(), 'checkout/success', array(
			'utm_nooverride' => 1
	...
	public function getFailedUrl($order_id){
		return str_replace('&amp;', '&',
				TrustPaymentsVersionHelper::createUrl($this->getCatalogUrl(), 'extension/trustpayments/transaction/fail',
						array(
							'order_id' => $order_id,
							'utm_nooverride' => 1
						), $this->registry->get('config')->get('config_secure')));
	...
	public function getWebhookUrl(){
		return TrustPaymentsVersionHelper::createUrl($this->getCatalogUrl(), 'extension/trustpayments/webhook', '',
				$this->registry->get('config')->get('config_secure'));
	}

	/**
	 * Checks if the given order_id exists, is associated with a trustpayments transaction, and the permissions to access it are set.
	 *
	...
	/**
	 * "trustpayments_pending_status_id"
	 * "trustpayments_processing_status_id"
	 * "trustpayments_failed_status_id"
	 * "trustpayments_voided_status_id"
	 * "trustpayments_decline_status_id"
	 * "trustpayments_fulfill_status_id"
	 * "trustpayments_confirmed_status_id"
	 * "trustpayments_authorized_status_id"
	 * "trustpayments_completed_status_id"
	 * "trustpayments_refund_status_id"
	 *
	 * @param string $order_id
	 * @param string|int $status Key for trustpayments status mapping, e.g. trustpayments_completed_status_id, or the order status id
	...
	public function ensurePaymentCode(array $order_info, \TrustPayments\Sdk\Model\Transaction $transaction){
		$allowed = $transaction->getAllowedPaymentMethodConfigurations();
		$code = null;
		if (count($allowed) === 1) {
			$code = 'trustpayments_' . $allowed[0];
		}
			...
			$code = 'trustpayments_' . $transaction->getPaymentConnectorConfiguration()->getPaymentMethodConfiguration()->getId();
		}
		else {
	...
	public static function extractPaymentMethodId($code){
		return substr($code, strlen('trustpayments_'));
	}
	...
	public static function getBaseUrl(){
		return rtrim("https://ep.trustpayments.com/", '/');
	}
