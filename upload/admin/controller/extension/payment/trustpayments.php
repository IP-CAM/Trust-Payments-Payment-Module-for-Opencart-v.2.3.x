...
class ControllerExtensionPaymentTrustPayments extends AbstractController {
	...
	public function index(){
		...
		$this->response->setOutput($this->loadView("extension/payment/trustpayments", array_merge($storeConfigs, $pageVariables)));
	}
	...
	private function getSettingPageVariables($shops){
		...
		// Form action url
		$data['action'] = $this->createUrl("extension/payment/trustpayments",
				array(
					\TrustPaymentsVersionHelper::TOKEN => $this->session->data[\TrustPaymentsVersionHelper::TOKEN]
				));
	...
	private function getSettingsPageBreadcrumbs(){
		return array(
			'breadcrumbs' => array(
				...
				array(
					"href" => $this->createUrl("extension/payment/trustpayments",
							array(
								\TrustPaymentsVersionHelper::TOKEN => $this->session->data[\TrustPaymentsVersionHelper::TOKEN]
							)),
	...
	private function validateStore($store){
		if (!$this->user->hasPermission("modify", "extension/payment/trustpayments")) {
	...
	protected function getRequiredPermission(){
		return 'extension/payment/trustpayments';
	}
}