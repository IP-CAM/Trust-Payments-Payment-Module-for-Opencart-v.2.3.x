...
class ControllerExtensionTrustPaymentsTransaction extends \TrustPayments\Controller\AbstractController {

	public function index(){
		$this->load->language('extension/payment/trustpayments');
		
		$this->document->setTitle($this->language->get('heading_transaction_list'));
		
		$this->load->model('extension/trustpayments/transaction');
	...
	protected function getList(){
		...
		$transactions = $this->model_extension_trustpayments_transaction->loadList($filters);
		$transactionCount = $this->model_extension_trustpayments_transaction->countRows();
		...
		$data['filterAction'] = TrustPaymentsVersionHelper::createUrl($this->url, 'extension/trustpayments/transaction',
				$this->getQueryString($filters), true);
		
		$data['order_statuses'] = $this->model_extension_trustpayments_transaction->getOrderStatuses();
		
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->loadView('extension/trustpayments/transaction', $data));
	}

	protected function getRequiredPermission(){
		return 'extension/trustpayments/transaction';
	}
	...
	private function getSortLinks($filters){
		...
		$sortUrl = TrustPaymentsVersionHelper::createUrl($this->url, 'extension/trustpayments/transaction', $query, true);
		$links = array();
	...
	private function getPagination(array $filters, $transactionCount){
		...
		$filters['page'] = '{page}';
		$pagination->url = TrustPaymentsVersionHelper::createUrl($this->url, 'extension/trustpayments/transaction',
				$this->getQueryString($filters), true);
		return $pagination;
	}
	...
	private function getBreadcrumbs(){
		return array(
			array(
				...
				'href' => TrustPaymentsVersionHelper::createUrl($this->url, 'sale/order',
						array(
			...
			array(
				...
				'href' => TrustPaymentsVersionHelper::createUrl($this->url, 'extension/trustpayments/transaction',
						array(
							'token' => $this->session->data['token'] 
						), true) 
			) 
